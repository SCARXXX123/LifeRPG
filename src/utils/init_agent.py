...
#         print(f"{color}[{timestamp}] [{status}] {message}{self.RESET}")
#
#     def generate_hero(self, background):
#         """
#         [初始化阶段] 将用户背景映射为 RPG 初始档案
#         """
#         self._log("AI_START", "正在唤醒 DeepSeek 建模专家...", self.BLUE)
#         start_time = time.time()
#
#         system_prompt = "你是一个只输出 JSON 的游戏系统初始化专家。"
#         user_prompt = f"""
#         你是一名顶级行为建模专家。请基于以下用户背景，初始化一个 RPG 风格的角色面板。
#         背景：{background}
#
#         输出纯 JSON：
#         {{
...
#             ],
#             "brief_report": "基于背景的初始审计简报（80字左右，公正尖锐）",
#             "system_comment": "犀利毒舌的中文系统寄语"
#         }}
#         """
#
#         try:
#             self._log("AI_THINK", "数据流已上传，正在解析用户基因 (等待 API 响应)...", self.YELLOW)
#
...
#
#             # 打印简要预览，让你确认数据没跑偏
#             self._log("DATA_PEEK", f"职业: {data.get('career')} | 技能数: {len(data.get('skills', []))}", self.BLUE)
#
#             return data
#
#         except json.JSONDecodeError:
#             self._log("AI_ERROR", "AI 返回了无效的 JSON 格式。", self.RED)
#             return None
#         except Exception as e:
#             self._log("AI_CRITICAL", f"通讯中断: {str(e)}", self.RED)
#             return None
...
class InitAgent:
    ...
    def generate_hero(self, background):
        ...
        必须返回如下格式的 JSON 字符串（严禁包含 Markdown 代码块或文字说明）：
        {{
            "career": "硬核赛博风格职业名",
            "level": 1,
            "radar_stats": {{ "智力": 10, "财力": 10, "力量": 10, "精力": 10, "魅力": 10, "韧性": 10 }},
